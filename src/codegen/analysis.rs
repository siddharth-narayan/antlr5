...
impl AntlrIR {
    ...
    pub fn nth(&mut self, alt: Arc<AltIR>, n: usize) -> BiMap<ElementIR, usize> {
        ...
        for (element_index, element) in elements.iter().enumerate() {
            match element.suffix() {
                Some(EBNFSuffix::Optional) | Some(EBNFSuffix::Star) => {
                    ...
                    nth_atoms.extend(self.internal_nth(alt.clone(), starting_element + element_index + 1, n - pos, depth + 1, &mut new_visited));
                },
                _ => ()
            };
            ...
            match element {
                ElementIR::RuleAtom { id, suffix } => {
                    if !visited.contains(id) {
                        ...
                        for alt in self.get_rule(*id).unwrap().alts().clone() {
                            let mut new_visited = visited.clone();
                            nth_atoms.extend(
                                self.internal_nth(alt, 0, n - pos, depth + 1, &mut new_visited).clone(),
                            )
                        }
                    }
                }
                ...
                ElementIR::Block { block, suffix: _ } => {
                    for alt in block {
                        let mut new_visited = visited.clone();
                        nth_atoms.extend(
                            self.internal_nth(alt.clone(), 0, n - pos, depth + 1, &mut new_visited)
                                .clone(),
                        )
                    }
                }
